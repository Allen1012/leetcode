...
//        String s = "aaaaaaacbbbbbb" ;
        int k = 301;
        int ret = l.longestSubstring(s,k);
    ...
    public int longestSubstring(String s, int k) {
        ...
        for (int i = length; i >= k ; i--) {
            if(isSubstring(s,k,i)){
                return i;
            }
        }
        return 0;

//        return binarySearchLen(s,k,0,s.length()/2,s.length());
    }

    public boolean isSubstring(String s,int k,int len){
        HashMap<Character,Integer> map = new HashMap<>();
        for (int i = 0; i < len; i++) {
            map.put(s.charAt(i),map.getOrDefault(s.charAt(i),0) + 1);
        }
        if(isAllBiggerK(map,k)){
            return true;
        }
    ...
    public boolean isAllBiggerK(HashMap<Character,Integer> map,int k){
        for (Integer mapVal: map.values()) {
            if(mapVal > 0 && mapVal < k){
                return false;
            }
        }
        return true;
